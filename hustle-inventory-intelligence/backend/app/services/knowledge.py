...
def generate_brief(topic: str, points: list[str]) -> str:
    ...
    brief = (
        ...
        f"{bullet_block}\n\n"
        "## Executive Direction\n\n"
        "Protect product availability, reduce slow-moving inventory drag, and tighten supplier resilience before expanding stock commitments.\n\n"
        f"{FOOTER}\n"
    )
    (REPORTS_DIR / "inventory_executive_brief.md").write_text(brief, encoding="utf-8")
    return brief
