...
def generate_brief(topic: str, points: list[str]) -> str:
    ...
    brief = (
        ...
        f"{bullet_block}\n\n"
        "## Executive Direction\n\n"
        "Prioritize mitigation where cash strain, dependency concentration, and compliance slippage intersect.\n\n"
        f"{FOOTER}\n"
    )
    (REPORTS_DIR / "risk_executive_brief.md").write_text(brief, encoding="utf-8")
    return brief
