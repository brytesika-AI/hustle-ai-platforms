...
def generate_brief(topic: str, points: list[str]) -> str:
    ...
    brief = (
        ...
        f"{bullet_block}\n\n"
        "## Executive Direction\n\n"
        "Protect pricing power, reduce churn drag, and concentrate expansion effort where commercial confidence is strongest.\n\n"
        f"{FOOTER}\n"
    )
    (REPORTS_DIR / "revenue_executive_brief.md").write_text(brief, encoding="utf-8")
    return brief
